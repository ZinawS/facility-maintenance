...
CREATE TABLE IF NOT EXISTS expenses (
  ...
  -- BIGINT, not INT: matches users.id's actual deployed type (the
  -- payments.user_id FK below has the same discrepancy — schema.sql says
  -- INT further up but every real database has users.id as BIGINT).
